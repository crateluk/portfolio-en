...
document.addEventListener('DOMContentLoaded',function(){var imgs=document.querySelectorAll('img[loading="lazy"]');imgs.forEach(function(img){if(img.closest('.c-card__header'))return;if(img.complete)return;var parent=img.parentElement;if(!parent)return;var cs=window.getComputedStyle(parent);if(cs.position==='static')parent.style.position='relative';var sp=document.createElement('div');sp.className='img-spinner';sp.innerHTML='<div class="wrapper"><div class="circle"></div><div class="circle"></div><div class="circle"></div><div class="shadow"></div><div class="shadow"></div><div class="shadow"></div></div>';parent.appendChild(sp);img.addEventListener('load',function(){sp.classList.add('hidden');});img.addEventListener('error',function(){sp.classList.add('hidden');});});});
